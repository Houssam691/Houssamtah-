...
export function sanitizeUser(user: User, viewerRole?: UserRole): SafeUser {
  const { password_hash, ...safe } = user;
  const result = safe as SafeUser;

  if (viewerRole !== "admin") {
    result.payment_rip = safe.payment_rip ? maskValue(safe.payment_rip) : "";
    result.payment_usdt_address = safe.payment_usdt_address ? maskValue(safe.payment_usdt_address) : "";
  }
