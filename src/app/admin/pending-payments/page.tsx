...
export default function AdminPendingPaymentsPage() {
  ...
  return (
    <div>
      <section className="glass rounded-3xl p-6 md:p-8">
        <h1 className="title">المدفوعات المستحقة للبائعين</h1>
        <p className="subtitle">طلبات انتهى ضمانها وأصبحت جاهزة للدفع للبائعين.</p>
      </section>
          ...
          <div key={o.id} className="glass rounded-3xl p-5">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 text-sm text-white/70">البائع: {o.seller_name}</div>
                <div className="mt-1 text-sm text-white/70">المبلغ: {o.total_amount} {o.currency}</div>
                {o.seller_rip && <div className="mt-1 text-sm text-white/70">RIP: {o.seller_rip}</div>}
                {o.seller_usdt && <div className="mt-1 text-sm text-white/70">USDT: {o.seller_usdt}</div>}
              </div>
            </div>
          </div>
        ))}
