...
type Stats = {
  ...
  top_sellers: { name: string; orders: number; revenue: number }[];
...
export default function AdminAnalyticsPage() {
  ...
  const cards = data?.cards ? [
    ...
    { label: "البائعين", value: data.cards.sellers, color: "text-purple-300" },
    ...
    <div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        ...
        <div className="glass rounded-3xl p-5">
          <div className="mb-4 text-sm font-black text-white/80">أفضل البائعين</div>
          <div className="grid gap-3">
            {data?.top_sellers?.map((s, i) => (
              <div key={i} className="flex items-center justify-between rounded-2xl bg-white/5 p-3">
                <div>
                  <div className="font-bold text-white">{s.name}</div>
                  <div className="text-xs text-white/50">{s.orders} طلب</div>
                </div>
                <div className="font-black text-emerald-300">{s.revenue.toLocaleString()} DZD</div>
              </div>
            ))}
