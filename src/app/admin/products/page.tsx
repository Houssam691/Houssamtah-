  ...
  return (
    <div>
      ...
      <div className="mt-3 flex flex-wrap items-end gap-2">
        ...
        {filtered.length > 0 && (
          <button className="btn-secondary text-sm h-10" onClick={() => downloadCSV(filtered.map(p => ({ title: p.title, category: p.category, price: p.price, currency: p.currency, status: p.status, seller: p.seller_name })), "products")}>CSV</button>
        )}
      </div>
        ...
        <table className="w-full text-right text-sm">
          <thead>
            <tr className="border-b border-white/10 text-xs font-bold text-white/50">
              ...
              <th className="pb-2 pl-2">البائع</th>
          ...
          <tbody>
            {filtered.map((p) => (
              <tr key={p.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                ...
                <td className="py-2 pl-2 text-xs text-white/60">{p.seller_name || "—"}</td>
