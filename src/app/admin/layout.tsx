...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen">
      {/* Mobile header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 flex h-14 items-center justify-between border-b border-white/10 bg-zinc-950/90 backdrop-blur-xl px-4">
      ...
      {/* Sidebar */}
      <aside className={`fixed top-0 right-0 z-50 h-full w-64 border-l border-white/10 bg-zinc-950/95 backdrop-blur-xl transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "translate-x-full"} overflow-y-auto`}>
        <div className="p-5">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={() => setSidebarOpen(false)} className="lg:hidden text-white/60">✕</button>
          </div>
          ...
          <a href="/" className="mt-6 flex items-center gap-2 rounded-2xl px-4 py-2.5 text-sm font-bold text-white/50 hover:text-white hover:bg-white/5 transition">
            <span>🏠</span>
