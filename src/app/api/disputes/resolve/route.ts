...
export async function POST(req: Request) {
  ...
  if (!["closed", "resolved_buyer", "resolved_seller"].includes(status)) {
  ...
  if (status === "resolved_buyer" || status === "closed") {
    ...
    await updateOrderStatus(order_id, "seller_paid");
  }
  ...
  if (d) {
    ...
    if (d.seller_id) {
      await createNotification({ userId: d.seller_id, orderId: order_id, type: "dispute_resolved", message: "تم إغلاق النزاع على طلب" });
    }
  }
