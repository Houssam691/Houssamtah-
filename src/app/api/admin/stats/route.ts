...
export async function GET(req: Request) {
  ...
  const totalSales = await db.queryOne<{ val: number }>("SELECT COALESCE(SUM(product_price),0) as val FROM orders WHERE status IN ('delivered','seller_paid')");
  const totalProfit = await db.queryOne<{ val: number }>("SELECT COALESCE(SUM(tax_amount),0) as val FROM orders WHERE status IN ('delivered','seller_paid')");
  const totalOrders = await db.queryOne<{ val: number }>("SELECT COUNT(*)::int as val FROM orders");
  const completedOrders = await db.queryOne<{ val: number }>("SELECT COUNT(*)::int as val FROM orders WHERE status IN ('delivered','seller_paid')");
  ...
  const sellers = await db.queryOne<{ val: number }>("SELECT COUNT(*)::int as val FROM users WHERE role = 'seller' AND seller_status = 'approved'");
  ...
  const dailySales = await db.query(`
    ...
    FROM orders WHERE status IN ('delivered','seller_paid') AND created_at >= NOW() - INTERVAL '30 days'
    GROUP BY DATE(created_at) ORDER BY label
  `);

  const topSellers = await db.query(`
    SELECT u.first_name || ' ' || u.last_name as name, COUNT(o.id)::int as orders, COALESCE(SUM(o.product_price),0) as revenue
    FROM orders o JOIN users u ON o.seller_id = u.id
    WHERE o.status IN ('delivered','seller_paid')
    GROUP BY o.seller_id ORDER BY revenue DESC LIMIT 5
  `);
