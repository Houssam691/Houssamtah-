...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const orders = await db.query(`
    ...
    FROM orders WHERE buyer_id = $1 OR seller_id = $1 ORDER BY created_at DESC LIMIT 20
  `, [id]);

  const products = await db.query(`
    SELECT id, title, category, price, currency, status, created_at
    FROM products WHERE seller_id = $1 ORDER BY created_at DESC LIMIT 20
  `, [id]);
  ...
  const reviews = await db.query(`
    ...
    WHERE r.buyer_id = $1 OR r.seller_id = $1 ORDER BY r.created_at DESC LIMIT 10
  `, [id]);
