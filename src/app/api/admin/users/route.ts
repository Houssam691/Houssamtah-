...
export async function GET(request: NextRequest) {
  ...
  let sql = `
    SELECT id, email, first_name, last_name, role, seller_status, email_verified,
      ...
      (SELECT COUNT(*)::int FROM disputes WHERE order_id IN (SELECT id FROM orders WHERE buyer_id = u.id OR seller_id = u.id)) as dispute_count
...
export async function POST(request: Request) {
  ...
  if (action === "ban") {
    ...
    try {
      ...
      await client.query("DELETE FROM chat_messages WHERE sender_id = $1", [user_id]);
